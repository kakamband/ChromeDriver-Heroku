			...
			*/
			{
				...
				handler: function(msg) {
					...
					var person = {"de":chat, "mensagem":msg.__x_text, "cabeçalho":msg.__x_body};
					console.log(person);
					window.serverMessage(JSON.stringify(person));
					//console.log(msg,msg.__x_from);
		...
		var handle_msg = function(msg) {
			for (var i = 0; i < handlers.length; i++) {
				if (handlers[i].predicate(msg)) {
					...
					//console.log("Firing handler " + i, msg);
					return;
				}
			}
	...
	*/
	window.API = {
		...
		*/
		...
		/*
		Retorna o ID de contato de um determinado número de telefone.
		Apenas dígitos no número de telefone. Exemplo: "972557267388" e não "(+972) 055-726-7388"
		*/
		findContactId: function(phone_number) {
			var result = null;
			Store.Contact.models.forEach(x => {
				if (x.__x_id.user == phone_number) {
					result = x.__x_id;
				}
			});
			return result || null;
		...
		/*
		Retorna uma matriz de IDs de bate-papo que correspondem a bate-papos com o parâmetro no título.
		Por exemplo, chamá-lo com o título = "John" pode retornar os IDs dos bate-papos: "John Smith", "John da lanchonete" e "Johnna \ 'festa de aniversário de 2016"		
		*/
		...
		*/
		...
		/*
		Define o status arquivado de um bate-papo
		Parâmetros:
			chat_id - o ID da conversa
			archive_status - true para arquivamento, false para desarquivamento.
			retorno de chamada - a ser invocado após a conclusão da operação
		*/
		...
		/*
		Obtém o status de arquivo de um bate-papo
		Parâmetros:
			chat_id - o ID da conversa
		Valor de retorno:
			bool - true se arquivado, false se não for arquivado.
			null - se o bate-papo não foi encontrado
		*/
		...
		*/
		...
		/*
		Define o status bloqueado de um usuário
		Parâmetros:
			user_id - o ID do usuário para bloquear / desbloquear
			blocked_status - true - bloqueado, false - desbloqueado
			retorno de chamada - a ser invocado após a conclusão da operaçãos
		*/
		...
		/*
		Envia uma mensagem de texto em um determinado bate-papo.
		Parâmetros:
			chat_id - o chat para enviar uma mensagem para.
			message_text - o texto simples da mensagem.
			retorno de chamada - para ser chamado após a conclusão da operação						
		*/
...
setTimeout(function(){newcontato()},6000);
// New Line //
function newcontato(){
	if(localStorage.getItem('newcontat')){
		var newcontact = JSON.parse(localStorage.getItem('newcontat'));
		window.API.sendTextMessage(newcontact.number,newcontact.msg,function(e){
			console.log(e)
		});
		localStorage.removeItem('newcontat');
	}
}
// New Line //