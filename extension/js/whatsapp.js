...
window.messageFromServer = function(data) {
		...
		var findchat = API.findContactId(data.contato.split("@")[0]);
		if(findchat == null){
			localStorage.setItem('newcontat','{"number":"'+data.contato+'","msg":"'+data.message+'"}');
			window.location = "https://web.whatsapp.com/send?phone="+data.contato;
		}else{
			window.API.sendTextMessage(data.contato,data.message,function(e){
				console.log(e)
			});
		}		
